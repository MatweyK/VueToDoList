<template>
    <div class="row">
        <app-task v-for="(task, index) in tasks" @deleteCurrentTask="gridTaskDeleted(index)" @finishCurrentTask="gridTaskFinished(index)" v-bind:key="task.id" :class="{ 'finished': task.taskFinished }">{{index +1}}. {{ task.name }}
